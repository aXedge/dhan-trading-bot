"""
NIFTY 50 Validation — Same 5 parameter sets on NIFTY 50 stocks
================================================================

Identical to midcap_validation.py but tests on NIFTY 50 constituents
to confirm whether Enhanced T (ADX 25) generalizes to large-caps.

Usage:
    python nifty50_validation.py
"""
...
# ---------------------------------------------------------------------------
# NIFTY 50 stocks (all 50 constituents)
# ---------------------------------------------------------------------------
...
NIFTY_50 = [
    ...
    "ONGC", "COALINDIA", "WIPRO", "HDFCLIFE", "SBILIFE",
    "TECHM", "DIVISLAB", "DRREDDY", "GRASIM", "CIPLA",
    "BAJAJFINSV", "ADANIPORTS", "JSWSTEEL", "SHRIRAMFIN", "ADANIENT",
    "BRITANNIA", "EICHERMOT", "HEROMOTOCO", "BAJAJ-AUTO", "TATACONSUM",
    "BPCL", "INDUSINDBK", "LICI", "DMART", "TATACONSUM",
]


# ---------------------------------------------------------------------------
# Indicators (same as midcap_validation.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Strategy (same as midcap_validation.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Backtest (same as midcap_validation.py)
# ---------------------------------------------------------------------------
...
def main():
    parser = argparse.ArgumentParser(
        description="Validate strategy on NIFTY 50 stocks.",
    )
    ...
    # Dedupe the list
    stocks = list(dict.fromkeys(NIFTY_50))

    print(f"\n{'='*70}")
    print(f"NIFTY 50 VALIDATION — {len(stocks)} stocks")
    print(f"{'='*70}")
    ...
    print(f"\n{'='*70}")
    print("SUMMARY — All parameter sets on NIFTY 50 stocks")
    print(f"{'='*70}")
    ...
    print(f"\n{'='*70}")
    print("CROSS-VALIDATION: NIFTY 50 vs NIFTY MIDCAP")
    print(f"{'='*70}")
    print(f"  {'Config':25s} {'N50 PF':>8s} {'N50 Tr':>7s} {'MID PF':>8s} {'MID Tr':>7s} {'Consistent?':>12s}")
    print(f"  {'-'*25} {'-'*8} {'-'*7} {'-'*8} {'-'*7} {'-'*12}")

    # Midcap results from previous run (hardcoded for comparison)
    midcap_results = {
        "Original T": {"pf": 0.79, "trades": 119},
        "Enhanced T (ADX 20)": {"pf": 0.98, "trades": 59},
        "Enhanced T (ADX 25)": {"pf": 1.07, "trades": 90},
        "Relaxed": {"pf": 0.86, "trades": 173},
        "Wide net": {"pf": 0.91, "trades": 253},
    }

    for set_name in PARAM_SETS:
        n50 = all_results[set_name]["metrics"]
        mid = midcap_results.get(set_name, {"pf": 0, "trades": 0})
        both_profitable = n50["pf"] >= 1.0 and mid["pf"] >= 1.0
        consistent = "YES" if both_profitable else "no"
        print(f"  {set_name:25s} {n50['pf']:8.2f} {n50['trades']:7d} {mid['pf']:8.2f} {mid['trades']:7d} {consistent:>12s}")
    ...
    if best_trades:
        ...
        sorted_stocks = sorted(per_stock.items(), key=lambda x: -x[1]["trades"])
    ...
    mid_pf = midcap_results.get(best_set, {}).get("pf", 0)

    if best_m["pf"] >= 1.0 and mid_pf >= 1.0 and best_m["trades"] >= 30:
        print(f"  Best config: {best_set}")
        print(f"  NIFTY 50: PF={best_m['pf']:.2f}, {best_m['trades']} trades, {best_m['win_rate']:.1f}% win rate")
        print(f"  NIFTY MIDCAP: PF={mid_pf:.2f}")
        print(f"  PROFITABLE ON BOTH BASKETS — strategy generalizes.")
        print(f"  Recommend deploying this config for paper trading.")
    elif best_m["pf"] >= 1.0 and best_m["trades"] >= 30:
        print(f"  Best config: {best_set}")
        print(f"  NIFTY 50: PF={best_m['pf']:.2f}, {best_m['trades']} trades")
        print(f"  NIFTY MIDCAP: PF={mid_pf:.2f}")
        print(f"  Profitable on NIFTY 50 but not confirmed on MIDCAP (or vice versa).")
        print(f"  Paper trade with caution.")
    else:
        print(f"  No config achieved PF >= 1.0 with 30+ trades on NIFTY 50.")
        print(f"  Best: {best_set} PF={best_m['pf']:.2f} trades={best_m['trades']}")
    print("=" * 70)
